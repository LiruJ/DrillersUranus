...
namespace WorldObjects
{
	...
	public:
		...
		void CentreOnMapObject(IReadOnlyMapObject& _mapObject) { m_worldPosition = ((_mapObject.GetTilePosition() * SpriteData::c_tileSize) + (SpriteData::c_tileSize / 2)) - (m_worldSize / 2); m_worldBounds.x = m_worldPosition.x; m_worldBounds.y = m_worldPosition.y; }
		...
		bool IsOnScreen(IReadOnlyMapObject& _mapObject) { return (m_worldBounds.IsPointInside(_mapObject.GetTilePosition() * SpriteData::c_tileSize) || m_worldBounds.IsPointInside((_mapObject.GetTilePosition() + 1) * SpriteData::c_tileSize)); }
	private:
